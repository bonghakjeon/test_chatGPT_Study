# 가상환경 폴더 "ch05_env" 생성 터미널 명령어
# python -m venv ch05_env

# 가상환경 폴더 "ch05_env" 활성화 터미널 명령어
# ch05_env\Scripts\activate.bat

# 비쥬얼스튜디오 코드(VSCode)
# streamlit 터미널 실행 명령어
# streamlit run 01_generate_ads_app.py
...
def main():
    ...
    with col1:  # 왼쪽공간 col1에 name, strenghth, keyword 작성(추가) -> 웹브라우저 화면에 데이터 시각화
        # 패키지 streamlit 함수 text_input 사용해서 text input을 받는 element를 생성했고
        # input 받은 text를 변수 name, strenghth, keyword에 저장 
    ...
    if st.button("광고 문구 생성"):   # 버튼 "광고 문구 생성" 생성 및 버튼 "광고 문구 생성" Click 이벤트 발생시 if문 실행 
        prompt = f'''
