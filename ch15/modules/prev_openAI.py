# [openai 이전 버전 전용 모듈] openai==0.28.1
...
def getMessageFromChunks(chunks, prompt):
    ...
    if prompt:
        ...
        # 아마존 웹서비스(AWS) 람다 함수(Lambda Function) 계층(Layer) 전용 faiss-cpu 패키지 layer.zip 파일 생성 
        ...
        # knowledge_base = FAISS.from_texts(chunks, embeddings)     # PDF 파일 청크(chunk) 단위 텍스트
        knowledge_base = FAISS.from_documents(chunks, embeddings)   # TEXT 파일 청크(chunk) 단위 텍스트
            
        docs = knowledge_base.similarity_search(prompt)
        
        print('[테스트] 질문과 유사한 응답 메시지: ', docs)
