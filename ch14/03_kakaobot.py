...
def mainChat(kakaorequest):
    ...
    # 잠시 저장하기 위한 텍스트 파일(.txt)을 파이썬 파일(03_kakaobot.py)과 같은 경로 안에
    # 텍스트 파일(.txt) "botlog.txt" 파일 이름으로 생성함.
    cwd = os.getcwd() # 파이썬 파일(03_kakaobot.py) 경로를 변수 cwd에 저장  
    filename = cwd + '/botlog.txt'  # 파이썬 파일(03_kakaobot.py) 경로에 '/botlog.txt' 파일 이름 생성하여 변수 filename에 저장 

    # 만약에 해당 텍스트 파일 (/botlog.txt)이 
    # 파이썬 파일(03_kakaobot.py) 경로에 없다면  
    if not os.path.exists(filename):
        # open 함수 사용하여 해당 텍스트 파일 (/botlog.txt) 쓰기모드("w")로 생성 
        with open(filename, "w") as f:
            f.write("")   # 처음에는 아무 것도 없는 값으로 해당 텍스트 파일 (/botlog.txt) 초기화 
    # 만약에 해당 텍스트 파일 (/botlog.txt)이 
    # 파이썬 파일(03_kakaobot.py) 경로에 있다면  
    else:
